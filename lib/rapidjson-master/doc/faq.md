...
1. What is RapidJSON?
   ...
   RapidJSON is a C++ library for parsing and generating JSON. You may check all [features](docs/features.md) of it.
...
4. What is *in situ* parsing?
   ...
   *in situ* parsing decodes the JSON strings directly into the input JSON. This is an optimization which can reduce memory consumption and improve performance, but the input JSON will be modified. Check [in-situ parsing](docs/dom.md) for details.

5. When does parsing generate an error?

   The parser generates an error when the input JSON contains invalid syntax, or a value can not be represented (a number is too big), or the handler of parsers terminate the parsing. Check [parse error](docs/dom.md) for details.
...
2. How to copy a value?
   ...
   There are two APIs: constructor with allocator, and `CopyFrom()`. See [Deep Copy Value](docs/tutorial.md) for an example.
...
4. What is BOM? How RapidJSON handle it?
   ...
   RapidJSON's `EncodedInputStream` can detect/consume BOM. `EncodedOutputStream` can optionally write a BOM. See [Encoded Streams](docs/stream.md) for example.
